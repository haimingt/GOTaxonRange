...
while(<IN>){
  ...
  if ($line =~ /relationship: (\w+) (\w+:\w+)/){
    ...
    my $c = $2;
    # CHEBI, CL, GOCHE, OBA, PR
    #     next unless (($line =~ /CHEBI|CL|GOCHE|OBA|PR|GO/) or ($goterm =~ /CHEBI|CL|GOCHE|OBA|PR/));
    #      $relation{$relation}{$goterm} .= $c.";";

    if (($goterm =~ /GO:/) and ($c =~ /CHEBI|CL|GOCHE|OBA|PR/)){
      $chebigo{$c}{$goterm} =1;
    }
    elsif (($c =~ /GO:/) and ($goterm =~ /CHEBI|CL|GOCHE|OBA|PR/)){
      $chebigo{$goterm}{$c} =1;
    }
    elsif (($goterm =~ /CHEBI|CL|GOCHE|OBA|PR/) and  ($c =~ /CHEBI|CL|GOCHE|OBA|PR/)){
      $reverse{$c} .= $goterm."*;";
      $reverse{$goterm} .= $c."*;";
    }
    elsif (($goterm =~ /CHEBI|CL|GOCHE|OBA|PR/)){
      $chebiother{$goterm}  .= $c.";";
    }
    elsif (($c =~ /CHEBI|CL|GOCHE|OBA|PR/)){
      $chebiother{$c}  .= $goterm.";";
    }
    else{
      #	print "$goterm: $line\n";
...
open IN, "< uberonConstraint.noname.txt" or die;
while(<IN>){
  my %single =eval($_);
...
print Dumper(\%chebigo);
#print "chebiother\n";
#print Dumper(\%chebiother);

exit;
my %YES;

open IN, "< ../go_yes.txt" or die;
while(<IN>){
  chomp;
  my @array = split(/\t/);
  $YES{$array[0]} = $array[1];
...
open IN, "< ../go_yes_funtaxis.txt" or die;
while(<IN>){
  chomp;
  my @array = split(/\t/);
  $YES{$array[0]} .= $array[1];
