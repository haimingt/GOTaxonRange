...
## this script is to help track how a GO term gets its constraints
...
while(<IN>){
	...
	$termType{$type}++;
	    ...
	    if ($goterm eq $limit){
		print "INPUT:$limit ; STRICT_RELATION: $1  $relation\n";
	    }
	  ...
	  if (($1 eq 'part_of') or ($1 eq 'occurs_in')){
	      ...
	      if ($goterm eq $limit){
		  print "INPUT:$limit ; STRICT_RELATION: $2  $relation\n";
	      }
	      ...
	      if ($goterm eq $limit){
		  print "INPUT:$limit ; OTHER_RELATION: $2  $relation\n";
	      }
	  }
      }
    }
...
foreach my $term1 (keys %strictRelation){
	...
	if ($strictRelation{$term1}{$term2} =~ /(\w+):\w+:NCBITaxon/){
		...
		if ($term2 =~ /NCBITaxon/){
		    ...
		    if ($term1 eq $limit){
			print "goCons type 2: $relation | $term2 \n";
		    }
...
while(<IN>){
  ...
  if ($key =~ /:/){
      ...
      if ($goterm eq $limit){
	  print "goCons type 4: Chebi |".$single{$key}."\n";
      }
...
foreach my $term1 (keys %strictRelation){
	...
	if ($uberonCon{$term2}){
	    ...
	    if ($term1 eq $limit){
		print "goCons type 1: uberon strictRelation |".$uberonCon{$term2}."\n";
	    }
...
foreach my $term1 (keys %otherRelation){
	...
	if ($uberonCon{$term2}){
	    ...
	    if ($term1 eq $limit){
		print "goCons type 1: uberon otherRelation |".$uberonCon{$term2}."\n";
	    }
...
sub combineCycle{
	...
	if ($goterm eq $limit){
	    print "$goterm in combineCycle\n";
#	    print "GetRoute: hashref print\n";
#	    print Dumper($hashref);
	...
	if ($goterm eq $limit){
	    print "Gain taxons from goCOns\n";
	    print Dumper(\%gaintaxons);
	    print "loss taxons from goCOns\n";
	    
	    print Dumper(\%losstaxons);
	    print "chebi taxons from goCOns\n";	    
	    print Dumper(\%chebitaxons);
	}
	...
	if (@gain){
	    ...
	    if ($goterm eq $limit){
		print "$limit constraint gain is $gain\n";
		print "$limit constraint gain source is $source\n";
		...
		if ($goterm eq $limit){
		    print "$limit constraint chebi is $gain\n";
		    print "$limit constraint chebi source is $source\n";
	...
	if (@loss){
	    ...
	    if ($goterm eq $limit){
		print "$limit constraint loss is $loss\n";
		print "$limit constraint loss source is $source\n";
	...
	if ($goterm eq $limit){
	    ...
	    print "$limit goCons: \n";
	    print Dumper($goCons{$goterm});
	    print "$limit combineCons: \n";
	    print Dumper($combineCons{$goterm});
	    print "$limit combine_construct: \n";
	    print Dumper($combine_construct{$goterm});
